...
We generate large programs with complex control-flows  by leveraging the isomorphism between control-flow graphs and context-free grammars. We next explain the program control-flow graphs.
A control-flow graph (CFG) is a representation of a program's execution flow used in software engineering for program analysis, optimization, and verification.

\subsection{Definition and Components}

A control-flow graph $G$ is a directed graph $G = (N, E)$ where:

\begin{itemize}
    \item $N$ is a set of nodes representing basic blocks of code. A basic block is a sequence of consecutive statements with:
    \begin{itemize}
        \item A single entry point (no jumps into the middle of the block)
        \item A single exit point (no jumps out of the middle of the block)
        \item Execution proceeding sequentially within the block
    \end{itemize}
    
    \item $E \subseteq N \times N$ is a set of directed edges representing possible transfers of control between basic blocks. Each edge $(n_i, n_j)$ indicates that control may flow from block $n_i$ to block $n_j$.
\end{itemize}

\subsection{Special Nodes}

A CFG typically contains special nodes:

\begin{itemize}
    \item \textbf{Entry node}: The unique node where execution of the procedure begins.
    \item \textbf{Exit node(s)}: Node(s) where execution of the procedure terminates.
\end{itemize}

\subsection{Edge Types}

Edges in a CFG may be labeled to indicate their nature:

\begin{itemize}
    \item \textbf{Unconditional edges}: Represent sequential flow or unconditional jumps.
    \item \textbf{Conditional edges}: Represent conditional branches (if-then-else, switch).
    \item \textbf{Loop edges}: Connect loop bodies back to loop conditions.
    \item \textbf{Exception edges}: Represent control transfers due to exception handling.
\end{itemize}
