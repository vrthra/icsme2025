...
We generate large programs with complex control-flows  by leveraging the isomorphism between control-flow graphs and context-free grammars. % We next explain the program control-flow graphs. %We explain what is a control-flow graph next.
A control-flow graph is a representation of a program's execution flow,
and is represented as a directed graph. Each individual procedure has a
separate control-flow graph, that is typically connected to other
control-flow graphs which are the call sites of that procedure
