...
\begin{subfigure}[h]{0.3\textwidth}
\begin{grammar}\centering
    <s> $\rightarrow$ <C> <IF> \phantom{a}
    
    <IF> $\rightarrow$ \term{T} <A> | \term{F} <B>
\end{grammar}
%\begin{grammar}\centering
%  <s> $\rightarrow$ <C> <IF>
 % 
...
\begin{subfigure}[h]{0.3\textwidth}
\begin{grammar}\centering
  <s> $\rightarrow$ <L>\phantom{d a b c d}
  
  <L> $\rightarrow$ <C> \term{T} <L> | <C> \term{F}
...
\begin{figure}
  ...
  \begin{tabular}{@{}p{1.2cm}@{ }|@{ }p{0.8\columnwidth}}
      ...
      %\newline \newline \text{for arbitrary threshold k,} \newline \text{\# infrequent species}  \quad  \newline \text{\# frequent species} \quad S(n)_{\text{\emph{freq}}} = \sum_{i>k}f_i(n) = S(n) - S(n)_{\text{\emph{infreq}}} \newline \text{sample coverage} \quad \hat C_{\text{\emph{infreq}}} = 1-f_1(n)\slash n \newline T_{\text{\emph{infreq}}} \quad \text{is the \# of inputs that belong to atleast one infrequent species}$\\
      \hline
...
(3) loops without conditionals (e.g. \mbox{\<while True: \ldots>})
(4) procedure calls.
As an example, we show in \Cref{fig:bsearch} how we translate \<bsearch> (\Cref{fig:bsearch1})
to this simplified form (\Cref{fig:bsearch2}).
